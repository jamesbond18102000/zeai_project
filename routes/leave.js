...
router.get("/approvers/:employeeId", async (req, res) => {
  try {
    ...
    // If the user is a HR, their approver is HR.
    else if (position === "hr") {
  ...
  try {
    ...
    if (approverRole === 'hr' || approverRole === 'founder') {
      findCondition.$or = [
        ...
        //only founders should see HR Leave requests
        ...(approverRole === 'founder' ? [{applicantRole:"hr"}]:[])
      ];
    } 
...
router.get("/pending-count", async (req, res) => {
    try {
        ...
        if (approverRole === 'superadmin' || approverRole === 'founder') {
            ...
            // ✅ HR should see leaves where approver is HR or assigned to HR
            if (!approverId) return res.status(400).json({ message: "❌ approverId is required for HR" });
