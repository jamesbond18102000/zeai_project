...
// ✅ Get employees for review (Founder → TL + HR, HR → TL only)
router.get("/employees/for-review/:employeeId", async (req, res) => {
  try {
    const { employeeId } = req.params;

    // 🔍 Find the logged-in user’s role automatically
    const user = await Employee.findOne({ employeeId: employeeId.trim() });
    if (!user) {
    ...
    if (userRole === "founder") {
      positionFilter = [/^TL$/i, /^HR$/i]; // Founder → see TL + HR
    } else if (userRole === "hr") {
      positionFilter = [/^TL$/i]; // HR → see TL only
    } else {
      positionFilter = []; // Others → nothing
    }
    ...
    console.error("❌ Error fetching employees for review:", err);
    res.status(500).json({ message: "❌ Server error fetching employees for review" });
  }
});
